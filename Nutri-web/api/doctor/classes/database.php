...
class Database
{
    ...
    public function register($user)
    {
        ...
        $sql = $this->connection->prepare(
            'INSERT INTO doctors (`name`, `lastname`, `username`, `password`, `email`, `status`, `created_date`) VALUES (?,?,?,?,?,?,?)'
        );
        $sql->bind_param(
            'sssssis',
            $user['name'],
    ...
    public function getUserByUsernameOrEmail($username)
    {
        ...
        $this->connection->set_charset('utf8');
        $sql = $this->connection->prepare(
            'SELECT DISTINCT * FROM `doctors` WHERE username=? OR email=?'
        );
    ...
    public function updateUser($user)
    {
        ...
        $this->connection->set_charset('utf8');
        $sql = $this->connection->prepare(
            'UPDATE `doctors` SET `name` = ?,`lastname`=?,`username`=?,`password`=?,`email`=? WHERE id=?'
        );
        $sql->bind_param(
            'sssssi',
            $user['name'],
